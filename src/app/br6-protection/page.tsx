...
import { getBR6Protection } from '@/content/queries'
import type { BR6Protection } from '@/content/types'
...
// International Certifications Section Component (with word animation)
const InternationalCertificationsSection: React.FC<{
  title: string
  description: string
...
// Threat Protection Matrix Section Component (with cards only)
const ThreatProtectionMatrixSection: React.FC<{
  title: string
  ...
  return (
    <section className="pt-[62px] pb-[50px] lg:pb-[90px] bg-[#560100] text-white overflow-hidden md:px-0 px-[15px]">
      ...
      {/* Cards Section - Desktop: Slider, Mobile: Column */}
      <motion.div
        variants={containerVariants}
        initial="hidden"
        whileInView="visible"
      ...
      >
        ...
        <div className="md:block hidden md:pl-6 lg:pl-[calc((100vw-1280px)/2+1.5rem)]">
            ...
            {cards.map((card, index) => (
              ...
              >
                <div className="flex flex-col">
                  ...
                  <p className="text-white/63 font-helvetica text-[14px] font-normal leading-[20px] m-0">
                    {card.description}
                  </p>
                </div>
              </motion.div>
...
export default function BR6ProtectionPage() {
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [protectionData, setProtectionData] = useState<BR6Protection | null>(null)
  const [currentIndex, setCurrentIndex] = useState(0)
  const [dragX, setDragX] = useState(0)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const protection = await getBR6Protection()
        console.log('BR6+ Protection data:', protection)
        setProtectionData(protection)
        setLoading(false)
      } catch (err) {
        console.error('Error fetching BR6+ Protection:', err)
        setError('Failed to load BR6+ Protection page')
        setLoading(false)
      }
    }
  ...
  const totalSlides = protectionData?.testingCertificationSlider.slides.length || 0
  ...
  if (loading) {
    return (
      <>
        ...
        <div className="br6-protection-container">
          <div style={{ textAlign: 'center', padding: '100px 0' }}>
            <div style={{ fontSize: '18px', color: '#6b7280' }}>Loading BR6+ Protection...</div>
          </div>
        </div>
      </>
    )
  ...
  if (error || !protectionData) {
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: criticalInlineStyles }} />
        <div className="br6-protection-container">
          <div style={{ textAlign: 'center', padding: '100px 0' }}>
            <div style={{ fontSize: '18px', color: '#ef4444' }}>{error || 'No BR6+ Protection data found'}</div>
          </div>
        </div>
      </>
    )
      ...
      {protectionData.testingCertificationSlider.slides.length > 0 && (
        <section className="pt-[50px] lg:pt-[79px] pb-[80px] lg:pb-[102px] bg-[#F4F1F2] text-black overflow-hidden mmd:px-0 px-[13px]" style={{ contain: 'layout style' }}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-[17px] lg:gap-16">
                  ...
                  >
                    <h2 className="md:text-left text-center text-black font-medium md:text-[64px] text-[30px] leading-[110%] tracking-[-1.28px] font-helvetica m-0">
                      {protectionData.testingCertificationSlider.mainName}
                    </h2>
                  </motion.div>
            ...
            >
              ...
              {totalSlides > 1 && (
                <div className="flex justify-center items-center gap-3 mt-8">
                  {protectionData.testingCertificationSlider.slides.map((_, index) => (
                    <button
                      key={index}
                      onClick={() => {
                        setCurrentIndex(index)
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 md:gap-[71px] gap-[18px]">
              <motion.div variants={itemVariants} className="lg:col-span-1 flex flex-col gap-[24px]">
                ...
                >
                  {protectionData.callToAction.title}
                </h1>

                {/* Desktop Button */}
                <div className="lg:col-span-1 space-y-8 md:block hidden">
                  <motion.a
                    href={protectionData.callToAction.buttonLink}
                  ...
                  >
                    ...
                    <span className="relative z-10 text-base lg:text-[14px] font-[700] transition-colors duration-500 ease-in-out group-hover:text-black">
                      {protectionData.callToAction.buttonText}
                    </span>
                  </motion.a>
                </div>
              </motion.div>
