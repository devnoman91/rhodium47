...
export default function CompareClient({ products }: CompareClientProps) {
  ...
  useEffect(() => {
    ...
    if (productHandles.length > 0) {
      ...
      setSelectedProducts(selected.length > 0 ? selected : allProducts.slice(0, 2))
    } else {
      setSelectedProducts(allProducts.slice(0, 2))
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      {/* Performance Section */}
      {selectedProducts.length > 0 && selectedProducts.some(p => p.specs) && (
            ...
            {selectedProducts.map((product, index) => (
              <div key={index} className="space-y-6">
                ...
                {product.specs.torque && (
                  <ComparisonRow
                    label="Torque"
                    value={product.specs.torque}
                  />
                )}
                {product.specs.towing && (
                  <ComparisonRow
                    label="Towing"
                    value={product.specs.towing}
                  />
                )}
                {product.specs.payload && (
                  <ComparisonRow
                    label="Payload"
                    value={product.specs.payload}
                  />
                )}
              </div>
            ))}
